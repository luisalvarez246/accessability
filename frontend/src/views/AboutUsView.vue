<template>
  <v-container fluid max-width="1140" class="pt-0 mx-auto">
    <v-container height="250" width="100%" class="hero_section d-flex justify-center pb-0 mb-0" color="mainbackground"
      rounded="lg">
      <v-img lazy-src="../assets/images/hero.avif" src="../assets/images/hero.avif" alt="" cover max-width="1140"
        max-height="250">
        <div class="title">
          <h2 class="underline pl-4">About Us</h2>
        </div>
      </v-img>
    </v-container>
    <v-container class="pt-0">
      <v-card class="team_members mx-auto" max-width="1140" color="cardbackground1">
        <v-card-title>The team</v-card-title>
        <v-row justify="center">
          <v-col cols="12" xs="12" sm="12" md="12" lg="8" xl="8">
            <v-row>
              <v-col v-for="author in authors" :key="author.id" cols="6" sm="4" lg="4">
                <v-card-item class="text-center justify-center align-center" color="cardbackground1">
                  <v-img :lazy-src="author.photo" :src="author.photo" :alt="author.name + ' ' + 'photo'" :width="120"
                    class="zoom-effect" />
                  <v-card-title class="team_members text-wrap pt-2">{{
                    author.name
                  }}</v-card-title>
                </v-card-item>
              </v-col>
            </v-row>
          </v-col>
        </v-row>
      </v-card>
      <v-card class="mt-4 mx-auto" max-width="1140" color="cardbackground1">
        <v-card-title> Contact </v-card-title>
        <v-divider></v-divider>
        <v-sheet class="mx-auto mb-10 px-10" width="90%" color="cardbackground2">
          <div class="mx-auto pt-3">
            <v-form role="form" class="pt-4">
              <v-text-field v-model="formData.fullName" :rules="[rules.required]" label="Full Name" id="input-9"
                type="text" autocomplete="name" aria-required="true" required bg-color="white" color="navbar"
                base-color="navbar" clearable></v-text-field>
              <v-text-field v-model="formData.email" :rules="[rules.required]" id="input-11" label="Email" type="text"
                autocomplete="email" aria-required="true" required bg-color="white" color="navbar" base-color="navbar"
                clearable></v-text-field>
            </v-form>
            <v-textarea v-model="formData.thoughts" :rules="[rules.required]" id="send-us-your-thoughts"
              name="send-us-your-thoughts" type="text" label="Send us your thoughts" aria-required="true" required
              auto-grow bg-color="white"></v-textarea>
          </div>
          <v-card-actions class="w-100 d-flex justify-center">
            <v-btn class="submit_btn text-none mb-4 px-6" rounded="xl" :block="xs" @click="submit"
              :size="xs ? '' : 'x-large'" :height="xs ? 52 : ''" :class="{ 'text-h6': xs }" color="navbar">
              Submit
            </v-btn>
          </v-card-actions>
        </v-sheet>
        <v-snackbar v-model="snackbar" :timeout="timeout" :color="snackbarColor">
          {{ snackbarMessage }}
          <template v-slot:actions>
            <v-btn color="navbar" variant="text" @click="snackbar = false">
              Close
            </v-btn>
          </template>
        </v-snackbar>
      </v-card>
    </v-container>
  </v-container>
</template>

<script setup>
import { ref } from "vue";
import { useDisplay } from "vuetify";

const authors = ref([
  {
    id: 1,
    name: "Carmen",
    photo: "/aboutus/images/Carmen.jpg",
  },
  {
    id: 2,
    name: "César",
    photo: "/aboutus/images/César.jpg",
  },
  {
    id: 3,
    name: "Leolo",
    photo: "/aboutus/images/Leolo.jpg",
  },
  {
    id: 4,
    name: "Luis",
    photo: "/aboutus/images/Luis.png",
  },
  {
    id: 5,
    name: "Miguel",
    photo: "/aboutus/images/Miguel.jpg",
  },
  {
    id: 6,
    name: "Sonia",
    photo: "/aboutus/images/Sonia.jpg",
  },
]);

const formData = ref({
  fullName: "",
  email: "",
  thoughts: "",
});

const rules = {
  required: (value) => !!value || "Field is required",
};

const { xs } = useDisplay();

const snackbar = ref(false);
const snackbarMessage = ref("");
const snackbarColor = ref("");
const timeout = ref(3000);

const validateForm = () => {
  return (
    rules.required(formData.value.fullName) &&
    rules.required(formData.value.email) &&
    rules.required(formData.value.thoughts)
  );
};

const submit = () => {
  if (validateForm()) {
    // lógica de envío de datos

    resetForm();
    showSnackbar("Formulario enviado con éxito", "success");
  } else {
    showSnackbar("Por favor, completa todos los campos obligatorios", "error");
  }
};

const resetForm = () => {
  formData.value.fullName = "";
  formData.value.email = "";
  formData.value.thoughts = "";
};

const showSnackbar = (message, color) => {
  snackbarMessage.value = message;
  snackbarColor.value = color;
  snackbar.value = true;
};
</script>
<style scoped>
.title {
  position: relative;
  color: #59029f;
}

div.title {
  background-color: #fed636;
  width: 25.5rem;
  height: 3.5rem;
  bottom: 0;
  content: "";
  display: block;
  left: 0;
  position: absolute;
  padding-top: 0.25rem;
}

.underline {
  position: relative;
}

h2.underline::after {
  background: #14cac9;
  bottom: 4;
  content: "";
  display: block;
  height: 0.25rem;
  left: 5;
  position: absolute;
  width: 5rem;
}

.v-card-title {
  font-size: 1.3rem;
  font-weight: 700;
}

.v-card-title.team_members {
  font-size: 1.1rem;
  font-weight: 500;
}

.zoom-effect {
  transition: transform 0.3s ease-in-out;
}

.zoom-effect:hover {
  transform: scale(1.1);
}

.v-btn.submit_btn {
  background: #fed636;
}
</style>
